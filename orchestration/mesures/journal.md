...
| TASK-097 clé d'API lue jusque dans les variables Windows | `orchestration/outils/resoudre-cle.sh` (nouveau), `lancer-agent.sh` (lignes 45-46), `tests/acceptance/TASK-097-cle.sh` | orchestrateur (session, en direct) — `orchestration/` interdit en écriture aux agents lancés | fichier de cas 0 à l'hôte (24) et 3 en conteneur (20, cas jouet indisponible, A182) ; lint conteneur 0 (128 fichiers) ; `verifier-liens.sh` 0 | deux mutants détectés (registre prioritaire : 3 échecs ; retour chariot conservé, en conteneur : 5 échecs) | 17 (script) + 3 (`lancer-agent.sh`) + 84 (cas) | — (aucun agent délégué) | Sonnet 7 appels, 24 350 jetons, 61 s | FUSIONNABLE ; 4 mineurs dont 3 corrigés (assertion creuse, niveaux de preuve, critère 4) ; A182 et A183 ouverts |
| TASK-098 anthropic.env à plusieurs modèles, `--modele` et `--dry-run` | `orchestration/modeles/anthropic.env`, `orchestration/outils/lancer-agent.sh` (149 lignes), `tests/acceptance/TASK-098-modeles.sh`, `orchestration/README.md` | orchestrateur (session, en direct) — `orchestration/` interdit en écriture aux agents lancés | fichier de cas 0 à l'hôte (29) et 3 en conteneur (22, lancement jouet indisponible, A182) ; non-régression TASK-097 0 (24) ; lint conteneur 0 (129 fichiers) ; `verifier-liens.sh` 0 | deux mutants détectés (`--modele` ignoré : 6 échecs ; tarifs du défaut conservés : 2) ; tarifs relevés à la source le 2026-09-19 | 149 (`lancer-agent.sh`, +22) + 14 (profil) + 111 (cas) | — (aucun agent délégué) | Sonnet 8 appels, 30 976 jetons, 81 s (abonnement) | FUSIONNABLE APRÈS CORRECTIONS ; 5 mineurs dont 4 corrigés (valeur de `--modele`, cas ajoutés, commentaire du cache) ; A184 ouvert |
| TASK-094 vues d'exécution, couches, registre des agents et inventaire dans `architecture.md` | `orchestration/architecture.md` (+106 lignes, §11 à 14) | `anthropic` Haiku 4.5 puis Sonnet 5 (API) — écriture et correction | `verifier-liens.sh` 0 ; `verifier-travail.sh` périmètre PASSE, validations 1/1 ; deux `VERDICT ECHEC` d'agent dus à A186, non au travail | six points de la relecture recontrôlés un à un (fautes, URLs, processus du sous-agent, classement, coûts « non mesuré ») ; sections 1 à 10 intactes | 106 (documentation) | Haiku 11 192 jetons de sortie, 0,147 $ ; Sonnet 20 690, 0,514 $ | Opus 5 par l'API, 13 tours, 9 113 jetons de sortie, 0,602 $ | NON FUSIONNABLE au premier jet (1 bloquant : coûts inventés ; 2 majeurs) ; corrigé, recontrôlé sans seconde relecture ; A185 à A187 ouverts ; coût total 1,263 $ |
| TASK-099 relecture lançable par l'API, interrupteur `relecture.json` | `orchestration/outils/lancer-agent.sh` (189 lignes, +40), `orchestration/relecture.json` (nouveau), `.claude/commands/tache.md` (étape 6), `orchestration/README.md`, `tests/acceptance/TASK-099-relecture.sh` (141 lignes) | orchestrateur (session, en direct) — `orchestration/` interdit en écriture aux agents lancés | fichier de cas 0 à l'hôte (37) ; non-régression TASK-098 0 (29) ; lint conteneur 0 (130 fichiers) ; `verifier-liens.sh` 0 ; `verifier-travail.sh` : périmètre PASSE, JUGE ÉCHEC (A169, connu, sans objet réel) | 189 (`lancer-agent.sh`, +40) + 141 (cas) | — (aucun agent délégué) | Opus 5 par l'API (`--relecture`, premier usage réel), 10 tours, 0,406 $, 160 s | FUSIONNABLE au premier jet ; A187 clos, A188 à A190 ouverts |
| TASK-095 extraire `lib-agents.sh` du générique de `lancer-agent.sh` | `orchestration/outils/lib-agents.sh` (nouveau, 128 lignes), `orchestration/outils/lancer-agent.sh` (189 → 150 lignes), `tests/acceptance/TASK-095-lib-agents.sh` (145 lignes), correctif non-régression sur TASK-097/098/099 (une ligne chacun) | `deepseek` (API) — première tâche sous décision 51, `orchestration/outils/` ouvert à l'agent lancé | fichier de cas 0 (40) ; non-régression 097/098/099 0 (24+29+37) ; lint conteneur 0 (132 fichiers) ; `verifier-travail.sh` : périmètre PASSE, JUGE ÉCHEC (A169, connu, sans objet réel) | deux majeurs (schéma agents.tsv enfermé dans la bibliothèque, preuve de lancement réel absente) corrigés en une relance ; deux mineurs (`shellcheck -x`, documentation) corrigés | 150 (`lancer-agent.sh`) + 128 (`lib-agents.sh`) + 145 (cas) | deepseek 0,185 $ (premier jet) + 0,122 $ (correction) = 0,307 $ | Opus 5 par l'API, 14 tours, 0,509 $, 175 s | FUSIONNABLE APRÈS CORRECTIONS ; corrigé en une relance, recontrôlé sans seconde relecture ; aucun Axx ouvert |
