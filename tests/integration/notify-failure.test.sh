...
# faux « curl » en tête de PATH, qui enregistre ses arguments et son entrée
# standard, et rend le verdict qu'on lui demande — aucune émission réelle. Il
# refuse d'émettre si l'URL ou le jeton se trouvent dans ses arguments, et le
# note : ils doivent passer par --config -, hors de la table des processus. La
# configuration ne peut pas vivre dans config/, zone interdite : un bac à sable
# recopie lib/common.sh et le script, et s'enracine de lui-même, SCRIPTS_ROOT se
# résolvant depuis l'emplacement de common.sh. L'identité des copies est prouvée
# avant usage.
...
# Le faux curl enregistre ses arguments (STUB_ARGS) et son entrée standard
# (STUB_CONF), puis agit selon STUB_MODE. Y trouver l'URL ou le jeton est une
# fuite : il la note dans STUB_FUITE et refuse d'émettre. Le mode « delai » ne
# dort que la borne reçue en argument : un appel non borné l'endort 10 s, ce que
# le cas du délai sanctionne. Aucune socket n'est ouverte.
cat > "$BAC/bin/curl" <<'STUB'
#!/bin/sh
printf '%s\n' "$*" >> "$STUB_ARGS"
for interdit in "$STUB_SECRET" "$STUB_JETON"; do
    [ -n "$interdit" ] || continue
    case "$*" in *"$interdit"*) printf '%s\n' "$interdit" >> "$STUB_FUITE"; exit 99 ;; esac
done
if [ "${1:-}" = "--config" ] && [ "${2:-}" = "-" ]; then cat > "$STUB_CONF"; fi
max=10
...
lancer() {
    local cible="$1"; shift
    SORTIE="$(STUB_ARGS="$BAC/appels" STUB_CONF="$BAC/conf" STUB_FUITE="$BAC/fuite" \
        STUB_SECRET="$SECRET" STUB_JETON="$JETON" STUB_MODE="$MODE" \
        PATH="$CHEMIN" LOG_DIR="$BAC/logs" \
        "$BASH_BIN" "$cible" "$@" 2>&1)" && CODE=0 || CODE=$?
}
# Ce que le faux curl a réellement reçu : arguments, puis entrée standard.
appels() { if [ -f "$BAC/appels" ]; then cat "$BAC/appels"; fi
           if [ -f "$BAC/conf" ]; then cat "$BAC/conf"; fi; }
# Trace laissée par toute fuite de l'URL ou du jeton dans les arguments de curl.
fuites() { if [ -f "$BAC/fuite" ]; then cat "$BAC/fuite"; fi; }
# Remet à zéro ce que le faux curl a vu : appels, entrée standard, fuites.
conf_vide() { rm -f "$BAC/appels" "$BAC/conf" "$BAC/fuite"; }
...
assert_contient "$SORTIE" "curl --config -"   "--help dit que l'URL ne passe pas en argument"
...
lancer "$CIBLE" --script 'update"system.sh' --code 1; assert_code 2 "$CODE" "un guillemet dans --script rend 2"
lancer "$CIBLE" --script 'update\system.sh' --code 1; assert_code 2 "$CODE" "une barre oblique inverse dans --script rend 2"
lancer "$CIBLE" --script update-system.sh --code 01; assert_code 2 "$CODE" "un zéro de tête dans --code rend 2"
...
config_ecrire ntfy "pas-une-adresse"
...
assert_absent "$SORTIE" "pas-une-adresse" "l'URL refusée n'est pas recopiée"
assert_egal "" "$(appels)" "aucune émission n'a été tentée"
...
assert_contient "$appel" "header = \"Authorization: Bearer $JETON\"" "le jeton part par l'entrée standard"
assert_egal "" "$(fuites)" "ni l'URL ni le jeton ne passent en argument de curl"
...
config_ecrire webhook; conf_vide
lancer "$BAC_SH" --script backup-resources.sh --code 2
assert_code 0 "$CODE" "le format webhook émet aussi"
appel="$(appels)"
assert_contient "$appel" "header = \"Content-Type: application/json\"" "le corps est annoncé comme du JSON"
assert_contient "$appel" "header = \"Authorization: Bearer $JETON\"" "le jeton vaut pour les deux formats"
...
assert_egal     "" "$(fuites)" "ni l'URL ni le jeton ne passent en argument de curl"
assert_absent   "$SORTIE" "sujet-tres-secret" "l'URL n'est pas affichée en webhook"
assert_absent   "$SORTIE" "$JETON" "le jeton n'est pas affiché en webhook"

titre "Échecs d'émission — la cause est nommée, le secret tu"
config_ecrire ntfy
...
assert_absent "$SORTIE" "sujet-tres-secret" "l'URL n'est pas dans la sortie d'échec"
assert_absent "$SORTIE" "$JETON" "le jeton n'est pas dans la sortie d'échec"
assert_egal   "" "$(fuites)" "ni l'URL ni le jeton ne passent en argument de curl"
MODE="muet"; lancer "$BAC_SH" --script update-system.sh --code 1
assert_code 1 "$CODE" "une réponse illisible rend 1"
assert_absent "$SORTIE" "sujet-tres-secret" "l'URL reste tu sur une réponse illisible"
assert_absent "$SORTIE" "$JETON" "le jeton reste tu sur une réponse illisible"
...
assert_absent   "$SORTIE" "sujet-tres-secret" "l'URL n'est pas dans le message de résolution"
assert_absent   "$SORTIE" "$JETON" "le jeton n'est pas dans le message de résolution"

titre "Délai borné — la borne vient de la configuration"
config_ecrire ntfy "$SECRET" 1; conf_vide
...
assert_absent "$SORTIE" "sujet-tres-secret" "l'URL n'est pas dans le message de délai"
assert_absent "$SORTIE" "$JETON" "le jeton n'est pas dans le message de délai"
...
assert_contient "$SORTIE" "[INFO] Méthode     : POST" "la méthode est affichée"
assert_contient "$SORTIE" "[INFO] Format      : ntfy" "le format est affiché"
...
assert_contient "$SORTIE" "[INFO] Destination : ntfy.exemple.test" "l'hôte de destination est affiché"
...
config_ecrire ntfy "https://ntfy.exemple.test/?sujet=secret-en-parametre"
lancer "$BAC_SH" --script update-system.sh --code 1 --dry-run
assert_contient "$SORTIE" "[INFO] Destination : ntfy.exemple.test" "une URL à paramètre n'affiche que l'hôte"
assert_absent   "$SORTIE" "secret-en-parametre" "le paramètre de l'URL n'est pas affiché"
config_ecrire ntfy "adresse-sans-schema-tres-secrete"
lancer "$BAC_SH" --script update-system.sh --code 1 --dry-run
assert_absent   "$SORTIE" "adresse-sans-schema-tres-secrete" "une URL sans schéma n'est jamais affichée en entier"
assert_contient "$SORTIE" "[INFO] Destination : non configurée" "aucun hôte n'est nommé à sa place"

titre "Le journal ne porte ni l'URL ni le jeton"
rm -f "$BAC/logs/notify-failure.log"; conf_vide; MODE="2xx"; config_ecrire ntfy
lancer "$BAC_SH" --script update-system.sh --code 1
...
titre "Le journal d'un échec les tait de même"
rm -f "$BAC/logs/notify-failure.log"; MODE="dns"
lancer "$BAC_SH" --script update-system.sh --code 1
journal=""; if [ -f "$BAC/logs/notify-failure.log" ]; then journal="$(cat "$BAC/logs/notify-failure.log")"; fi
assert_non_vide "$journal" "l'échec est journalisé"
assert_contient "$journal" "ntfy.exemple.test" "seul l'hôte y figure"
assert_absent "$journal" "sujet-tres-secret" "le sujet ntfy n'est pas journalisé sur échec"
assert_absent "$journal" "$JETON" "le jeton n'est pas journalisé sur échec"
...
assert_contient "$exemple" "les DEUX formats" "le modèle dit à quel format le jeton s'applique"
