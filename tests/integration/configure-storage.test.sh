...
CIBLE="$SCRIPTS_ROOT/Kubernetes/Configuration/configure-storage.sh"
if [ ! -e /.dockerenv ]; then
...
K='storageclass.kubernetes.io/is-default-class'
case "$1" in
get)
    ...
    for f in "$BAC"/classes/*; do
        ...
        [ "${KUBECTL_DERIVE:-}" != "$n" ] || [ ! -f "$BAC/avant-$n" ] || v="$(cat "$BAC/avant-$n")"
        [ "$v" = "absent" ] && v=""
        printf '%s %s\n' "$n" "$v"
    done ;;
annotate)
    ...
    n="$3"
    case "$4" in "$K=true") v=true ;; "$K=false") v=false ;;
...
# L'état du cluster : un fichier par classe, portant la valeur de l'annotation.
etat() { rm -rf "$BAC/classes"; mkdir -p "$BAC/classes"; for c in "$@"; do printf '%s\n' "${c#*=}" > "$BAC/classes/${c%%=*}"; done; }
...
for m in "Local-Path" "local path" "local_path" "-local" "local-" "local..path" "a--b" "local.path."; do
    CLASSE="$m"; lancer --yes
...
assert_egal "true" "$(cat "$BAC/classes/longhorn")" "l'état du cluster est resté intact"
...
assert_egal "true|false" "$(cat "$BAC/classes/local-path")|$(cat "$BAC/classes/longhorn")" "l'état final est celui voulu"
...
assert_egal "true" "$(cat "$BAC/classes/local-path")" "la cible a été marquée avant l'échec : jamais zéro classe par défaut"
assert_egal "false" "$(cat "$BAC/classes/longhorn")" "et l'autre a bien été démarquée avant le refus"
...
titre "Mutations — la suite doit les voir, sinon elle ne prouve rien"
mkdir -p "$BAC/mut/Kubernetes/Configuration"; ln -sfn "$SCRIPTS_ROOT/lib" "$BAC/mut/lib"
MUT="$BAC/mut/Kubernetes/Configuration/configure-storage.sh"
# Journal à part : ces exécutions sont fautives par construction, et saliraient
# les relevés de « Ce que le script ne fait jamais ».
lancer_mut() { etat local-path=absent longhorn=true; sain; : > "$BAC/kubectl-mut"
               sortie="$(env PATH="$CHEMIN" KUBECTL_JOURNAL="$BAC/kubectl-mut" "${BASE[@]}" "${EXTRA[@]}" timeout 60 bash "$MUT" "$@" </dev/null 2>&1)" && CODE=0 || CODE=$?; }
mut_annotes() { grep -c -- '^kubectl annotate' "$BAC/kubectl-mut" || true; }
cp "$CIBLE" "$MUT"; lancer_mut --yes
assert_code 0 "$CODE" "copie intacte : le chemin nominal rend 0"
sed -e "s#^grep -qxF \"\$CIBLE\".*\$#:#" -e "s#^for d in .*\$#&; PLAN+=(\"\$CIBLE=true\")#" "$CIBLE" > "$MUT"
lancer_mut --yes
assert_code 0 "$CODE" "ordre inversé : le mutant annote quand même, en 0"
assert_contient "$(grep -- '^kubectl annotate' "$BAC/kubectl-mut" | head -1)" "longhorn" "et la suite voit que la cible n'est plus annotée en premier"
sed 's/^\( *\)appel annotate storageclass/\1:/' "$CIBLE" > "$MUT"
lancer_mut --yes
assert_code 1 "$CODE" "annotate retiré : la relecture finale rend 1"; assert_egal "0" "$(mut_annotes)" "et aucun annotate n'a eu lieu"
sed -e 's/^\[ -t 0 \].*$/:/' -e 's/^confirm "Appliquer.*$/:/' "$CIBLE" > "$MUT"
lancer_mut
assert_code 0 "$CODE" "garde de confirmation retirée : le mutant annote sans terminal"
assert_egal "2" "$(mut_annotes)" "et la suite voit les annotations posées sans confirmation"
...
assert_egal "0" "$(grep -- '^kubectl annotate' "$BAC/kubectl-tous" | grep -vc ' storageclass.kubernetes.io/is-default-class=' || true)" "toute annotation porte exactement la clé par défaut"
