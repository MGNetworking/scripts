#!/usr/bin/env bash
set -Eeuo pipefail
# tests/integration/security-check.test.sh — Linux/Security/security-check.sh
# (TASK-043). Les cinq outils interrogés sont remplacés par un faux binaire.
...
detail_de() { sed -n "s/^[A-Z]*  $1 — //p" "$F_OUT"; }
...
if [ ! -f "$CIBLE" ]; then
    ko "Linux/Security/security-check.sh existe" "fichier introuvable : $CIBLE"
    bilan "TASK-043 / security-check.sh"
fi
if [ "$(uname -s 2>/dev/null || true)" != "Linux" ]; then
    saute_par_nature "l'ensemble des cas de security-check.sh" "cet hôte n'est pas un Linux"
    bilan "TASK-043 / security-check.sh"
fi
...
case "${0##*/}" in
    sshd) [ -z "${SSHD_DORT:-}" ] || sleep "$SSHD_DORT"
          printf 'port 22\npasswordauthentication %s\npermitrootlogin %s\n' "${SSH_MDP:-no}" "${SSH_ROOT:-no}" ;;
    ufw)  if [ "${UFW_ACTIF:-oui}" = "oui" ]; then
              printf 'Status: active\n\nTo  Action  From\n--  ------  ----\n22/tcp  ALLOW  Anywhere\n\nDefault: %s (incoming), allow (outgoing), disabled (routed)\n' "${UFW_ENTREE:-deny}"
    ...
    apt-get) [ "$*" = "-s upgrade" ] || exit 2
             n="${APT_MAJ:-0}"; i=0; printf 'NOTE: ceci n est qu une simulation.\n'
             while [ "$i" -lt "$n" ]; do i=$((i + 1)); printf 'Inst paquet%s (1.0 deb) []\n' "$i"; done ;;
    *) exit 127 ;;
esac
FAUX_OUTIL
...
for motif in "PASS, WARNING, FAIL ou INFO" "sshd -T" "ufw" "fail2ban" "UID 0" "apt-get -s upgrade" "BORNE" "Codes de retour"; do assert_contient "$(sortie)" "$motif" "--help documente « $motif »"; done
...
titre "4. Pare-feu — ufw actif et deny en entrée"
...
if PATH="$SANS" command -v sshd ufw fail2ban-client getent apt-get >/dev/null 2>&1; then ko "garde : les cinq outils sont masqués" "l'un d'eux reste visible"
else ok "garde : les cinq outils sont masqués"; fi
...
lancer bash "$CIBLE"
NB_FAIL="$(sed -n 's/^Bilan : .*, \([0-9]*\) FAIL.*$/\1/p' "$F_OUT")"
if [ "${NB_FAIL:-0}" -gt 0 ]; then assert_code 1 "$CODE" "un FAIL de la machine réelle rend le code 1"
else assert_code 0 "$CODE" "aucun FAIL sur la machine réelle : le code reste 0"; fi
