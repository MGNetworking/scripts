...
# telle que le vrai la rend, et ses messages d'erreur, codes compris.
faux kubectl <<'EOF'
#!/bin/sh
printf 'kubectl %s\n' "$*" >> "$BAC/kubectl-appels"; printf 'kubectl %s\n' "$*" >> "$BAC/kubectl-tous"
[ -z "${KUBECTL_ERREUR:-}" ] || { printf '%s\n' "$KUBECTL_ERREUR" >&2; exit "${KUBECTL_CODE:-1}"; }
case "$1 $2" in
    "get ingressclasses") [ -f "$BAC/ingressclasses" ] || { printf 'Error from server (NotFound): the server could not find the requested resource\n' >&2; exit 1; }; cat "$BAC/ingressclasses" ;;
    "get deployment") [ -f "$BAC/deploiement" ] || { printf 'Error from server (NotFound): deployments.apps "traefik" not found\n' >&2; exit 1; }; cat "$BAC/deploiement" ;;
    "get services") cat "$BAC/services" ;;
...
sain; EXTRA=(KUBECTL_ERREUR='error: error loading config file "/root/.kube/config": no such file or directory'); lancer; EXTRA=()
...
assert_contient "$sortie" "Aucune écoute visible par ss" "et le script dit ce que ss ne voit pas (servicelb, hostPort)"
...
assert_contient "$(cat "$BAC/ss-appels")" "ss -ltn" "et ss est appelé sans -p"
