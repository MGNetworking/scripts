...
# MODIFIE LE SYSTÈME : compte, groupe et groupe « sudo » de l'image, retirés par un trap armé en
# conteneur jetable seulement. Le mot de passe est jugé sur /etc/shadow, « $ » en tête valant
# condensé posé ; l'idempotence passe par empreinte, sous la garde « avant != après ».
...
# Groupe distinct du compte : useradd pose lui-même un groupe privé du nom du compte.
...
# Le groupe « sudo » existe dans cette image (base-passwd le pose), contrairement à ce que
# supposait la fiche : le refus ne s'éprouve qu'en le retirant, GID relevé pour le rendre intact.
SUDO_GID=""; SUDO_PREEXISTAIT="false"
if getent group sudo >/dev/null 2>&1; then SUDO_PREEXISTAIT="true"
    if ! SUDO_GID="$(getent group sudo | cut -d: -f3)"; then SUDO_GID=""; fi; fi

filet_de_securite() {
    userdel -r "$COMPTE" >/dev/null 2>&1 || true; userdel -r "$COMPTE_DRY" >/dev/null 2>&1 || true
    groupdel "$GROUPE" >/dev/null 2>&1 || true
    if [ "$SUDO_PREEXISTAIT" = "false" ]; then groupdel sudo >/dev/null 2>&1 || true
    elif ! getent group sudo >/dev/null 2>&1; then groupadd -g "$SUDO_GID" sudo; fi
    rm -f "$SUDOERS" "$SUDOERS_DRY" /etc/sudoers.d/.mgnetworking-* >/dev/null 2>&1 || true
    rmdir /etc/sudoers.d >/dev/null 2>&1 || true; rm -rf "$REP_TMP" /tmp/mgnet-test-users-nobody
}
...
membre() {
    local vu="non"; if id -nG "$1" | grep -qw -- "$2"; then vu="oui"; fi
    if [ "$vu" = "$3" ]; then ok "$4"; else ko "$4" "groupes : $(id -nG "$1")"; fi
}
assert_compte_absent() { if id -u "$1" >/dev/null 2>&1; then ko "$2" "le compte $1 existe"; else ok "$2"; fi; }
# Empreinte de ce qu'une création de compte touche : fichiers de comptes, CONTENU de /etc/sudoers.d, homes.
empreinte() {
...
# noter <variable> — empreinte prise en contexte de condition (TASK-018) ; printf -v affecte par nom (SC2154).
noter() { if ! printf -v "$1" '%s' "$(empreinte)"; then printf -v "$1" ''; fi; }
avant=""; apres=""; apres2=""; complet_avant=""; complet_apres=""; dry_avant=""; dry_apres=""
assert_empreinte() {
    if [ "$1" != "$2" ] && [ "$3" = "egal" ]; then ko "$4" "l'état a changé : $1 puis $2"
    elif [ "$1" = "$2" ] && [ "$3" = "different" ]; then ko "$4" "aucune modification relevée : la preuve serait vide"
    else ok "$4"; fi
}
...
ordre() {
    local a="" b=""
    a="$(rang "$1")" || a=""; b="$(rang "$2")" || b=""
    if [ -n "$a" ] && [ -n "$b" ] && [ "$a" -lt "$b" ]; then ok "$3"
    else ko "$3" "« $1 » ligne ${a:-absente}, « $2 » ligne ${b:-absente}"; fi
}

EST_LINUX="false"; case "$(uname -s 2>/dev/null)" in Linux) EST_LINUX="true" ;; esac
EST_DEBIAN="false"; if [ -r /etc/os-release ]; then case "$(sed -n 's/^ID=//p' /etc/os-release | tr -d '"')" in debian|ubuntu) EST_DEBIAN="true" ;; esac; fi
...
elif [ "${MGNET_TEST_JETABLE:-}" = "1" ]; then JETABLE="true"; fi
# Lanceur non privilégié : le premier qui abaisse RÉELLEMENT l'UID — sinon ces cas sont NON EXÉCUTÉS.
abaisse() { [ "$("$@" id -u 2>/dev/null)" = "65534" ]; }; LANCEUR_SANS_ROOT=()
if [ "$EST_ROOT" = "false" ]; then LANCEUR_SANS_ROOT=(env)
elif abaisse setpriv --reuid=65534 --regid=65534 --clear-groups; then LANCEUR_SANS_ROOT=(setpriv --reuid=65534 --regid=65534 --clear-groups)
elif abaisse runuser -u nobody --; then LANCEUR_SANS_ROOT=(runuser -u nobody --)
fi
...
MODIFIANT="oui"
if [ "$EST_ROOT" != "true" ]; then MODIFIANT="require_root arrête le script avant toute écriture"
elif [ "$JETABLE" != "true" ]; then MODIFIANT="cet hôte n'est pas un système jetable — ni /etc/passwd ni /etc/sudoers.d ne seront touchés"
elif [ "$EST_DEBIAN" != "true" ]; then MODIFIANT="l'hôte n'est ni Debian ni Ubuntu — require_os refuse le script"; fi
...
CLE="$REP_TMP/essai.pub"; SECONDE="$REP_TMP/seconde.pub"
...
aide=""; if ! aide="$(cat "$F_OUT")"; then aide=""; fi
for motif in "--utilisateur" "--cle-fichier" "--groupe" "--sudo" "--sudo-sans-mot-de-passe" "--shell" "--dry-run" \
             "Ce que ce script ne fait pas" "élévation interactive" "passwd"; do assert_contient "$aide" "$motif" "--help documente « $motif »"; done

titre "2. Usage refusé — code 2, et aucun compte créé"
# refus <libellé> <cible> <motif attendu> [arguments...] — la cible vide n'a pas de compte à prouver absent.
refus() {
    local libelle="$1" cible="$2" motif="$3"; shift 3
    lancer bash "$CIBLE" "$@"; assert_code 2 "$CODE" "$libelle"; assert_contient "$(erreur)" "$motif" "$libelle : le motif du refus est nommé"
...
if [ "${#LANCEUR_SANS_ROOT[@]}" -eq 0 ]; then
    ...
    saute "la primauté de l'erreur d'usage sur le manque de privilège" "aucun lanceur ne parvient à abaisser l'UID"
else
...
if [ "$MODIFIANT" != "oui" ]; then
    ...
    # Le groupe sudo est retiré : c'est le cas nominal du refus, et il ne s'éprouve qu'ICI.
    groupdel sudo >/dev/null 2>&1 || true
    lancer bash "$CIBLE" --utilisateur "$COMPTE" --sudo; assert_code 1 "$CODE" "le groupe sudo absent fait sortir en 1"
    ...
    if getent group sudo >/dev/null 2>&1; then ko "le groupe sudo n'a pas été créé" "le groupe existe désormais"; else ok "le groupe sudo n'a pas été créé"; fi
    lancer bash "$CIBLE" --utilisateur "$COMPTE" --groupe mgnetabsent; assert_code 1 "$CODE" "un groupe demandé inexistant fait sortir en 1"
...
if [ "$MODIFIANT" != "oui" ]; then saute "la création du compte et le dépôt de la clé" "$MODIFIANT"
else
    groupadd "$GROUPE"; if [ -n "$SUDO_GID" ]; then groupadd -g "$SUDO_GID" sudo; else groupadd sudo; fi
    noter avant; lancer bash "$CIBLE" --utilisateur "$COMPTE" --cle-fichier "$CLE" --groupe "$GROUPE"; assert_code 0 "$CODE" "la création nominale sort en 0"
...
if [ "$MODIFIANT" != "oui" ] || [ ! -f "$AUTORISE" ]; then saute "la seconde exécution ne modifie rien" "$MODIFIANT sans création préalable"
else
...
if [ "$MODIFIANT" != "oui" ]; then saute "--dry-run n'écrit rien" "$MODIFIANT"
else
    ...
    noter dry_apres; assert_empreinte "$dry_avant" "$dry_apres" egal "--dry-run ne modifie aucun fichier de comptes"
fi
