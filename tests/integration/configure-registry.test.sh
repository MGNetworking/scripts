...
# vrai. Il consigne ses arguments, relève « ps -eo args » et l'environnement de
# ses ascendants pendant l'appel, garde le manifeste reçu et le décode : c'est ce
# qui prouve que le jeton part — dans le Secret — et ne part jamais ailleurs.
# L'état du cluster tient dans un fichier par namespace : deux exécutions
# enchaînées se lisent l'une l'autre.
#
# config/registry.env vit dans config/, zone du dépôt : le bac recopie
# lib/common.sh et le script, et s'enracine de lui-même. Les valeurs sentinelles
# ne passent ni par l'environnement ni par un argument : un fichier du bac les
# porte, que le faux kubectl lit.
...
mkdir -p "$BAC/src/lib" "$BAC/src/config" "$BAC/src/Kubernetes/Configuration" \
         "$BAC/bin" "$BAC/bin-nu" "$BAC/etat" "$BAC/recu" "$BAC/tmp" "$BAC/logs" "$BAC/tmo"
...
: > "$BAC/attendu"; : > "$BAC/environ"
# Le vrai timeout, relevé avant toute retouche du PATH : lancer() l'appelle par
# son chemin absolu, un faux timeout ne peut donc jamais l'attraper.
REEL_TIMEOUT="$(command -v timeout)"
faux() { cat > "$BAC/bin/$1"; chmod +x "$BAC/bin/$1"; }
for outil in bash sh id mkdir basename dirname date uname cat tee sed head rm mktemp grep tr stat base64 sha256sum ps script awk sort cut find; do
...
# Environnement des ascendants — le faux, le timeout, le script, le bac : si
# allexport laissait le jeton exporté, il serait dans l'un de ces /proc.
p="$$"
while [ -n "$p" ] && [ "$p" != 0 ] && [ "$p" != 1 ]; do
    tr '\0' '\n' < "/proc/$p/environ" 2>/dev/null >> "$BAC/environ"
...
"apply -f")
    ...
    # valeur tout en chiffres, ou de forme 123e456, comme un nombre.
    emp="$(sed -n 's/^    mgnetworking\/empreinte: "\(.*\)"$/\1/p' "$m")"
...
CHEMIN="$BAC/bin:$PATH"
BASE=(DELAI_TEST=5 LOG_DIR="$BAC/logs" TMPDIR="$BAC/tmp"); EXTRA=(); INTERPRETE=()
LISTE="web,data,monitoring"; CODE=0
...
# « timeout » est appelé par son chemin absolu : le « timeout 60 » de la suite ne
# peut donc jamais être résolu vers le faux timeout du cas « Délai dépassé ».
lancer() { sortie="$(env PATH="$CHEMIN" SRV_K8S_NAMESPACES="$LISTE" "${BASE[@]}" "${EXTRA[@]}" "$REEL_TIMEOUT" 60 bash "${INTERPRETE[@]}" "$SRC" "$@" </dev/null 2>&1)" && CODE=0 || CODE=$?
           printf '%s\n' "$sortie" >> "$BAC/sorties"; }
# Réponse tapée sous un pseudo-terminal : le terrain de la décision 45.
lancer_pty() { rep="$1"; shift; sortie="$(printf '%s\n' "$rep" | env PATH="$CHEMIN" SRV_K8S_NAMESPACES="$LISTE" "${BASE[@]}" "${EXTRA[@]}" "$REEL_TIMEOUT" 60 script -qec "bash $SRC $*" /dev/null 2>&1)" && CODE=0 || CODE=$?
               printf '%s\n' "$sortie" >> "$BAC/sorties"; }
sain() { : > "$BAC/kubectl-appels"; : > "$BAC/ps"; : > "$BAC/fuite"; : > "$BAC/environ"
         rm -rf "$BAC/etat"; mkdir -p "$BAC/etat"; }
...
assert_egal "" "$(appels)" "aucun appel kubectl n'a eu lieu"
# Une forme par défaut de la grammaire hôte[:port] : schéma, barre oblique, blanc,
# double deux-points, souligné, tiret en tête ou en queue d'étiquette, port vide
# ou non numérique.
for m in "https://registry.exemple.test" "registry.exemple.test/v2" "registry exemple.test" \
         "::" "registry_exemple.test" "-registry.exemple.test" "a-.b" \
         "registry.exemple.test:" "registry.exemple.test:abc"; do
    config "$m"; sain; lancer --yes
    assert_code 2 "$CODE" "l'adresse « $m » est refusée en 2"
    assert_absent "$sortie" "$m" "l'adresse « $m » n'est pas recopiée dans le refus"
done
assert_contient "$sortie" "mal formé" "le refus dit ce qui est mal formé"
LISTE="web"; config "registry.exemple.test"; sain; EXTRA=(CLUSTER_NS="web"); lancer --yes; EXTRA=()
assert_code 0 "$CODE" "un hôte seul, sans port, reste accepté"
LISTE="web,data,monitoring"; config
...
assert_egal "" "$(appels)" "aucun appel kubectl n'a eu lieu"
LONG="$(printf 'a%.0s' {1..64})"; LISTE="$LONG"; sain; lancer --yes; LISTE="web,data,monitoring"
assert_code 2 "$CODE" "un nom de 64 caractères est refusé en 2"
assert_contient "$sortie" "63 caractères" "le refus dit la limite RFC 1123"
assert_egal "" "$(appels)" "aucun appel kubectl n'a eu lieu pour un nom trop long"
LONG="$(printf 'a%.0s' {1..63})"; LISTE="$LONG"; sain; EXTRA=(CLUSTER_NS="$LONG"); lancer --yes; EXTRA=()
assert_code 0 "$CODE" "un nom de 63 caractères, la limite, est accepté"
LISTE="web,data,monitoring"
...
# Faux timeout : fichier ORDINAIRE, dans un répertoire propre au cas et à
# l'intérieur du bac — jamais un lien, jamais hors de $BAC. Le vrai binaire n'est
# pas touché, et « lancer » l'appelle par son chemin absolu : le faux ne peut ni
# le remplacer ni s'attraper lui-même. Pour l'appel visé il rend 124 sans
# attendre ; sinon il exécute la commande directement, sans passer par le vrai.
cat > "$BAC/tmo/timeout" <<'SH'
#!/bin/sh
printf '%s\n' "$*" >> "$BAC/timeout-appels"
case "${TIMEOUT_SUR:-}:$*" in
    get:*" kubectl get "*) exit 124 ;;
    apply:*" kubectl apply "*) exit 124 ;;
esac
shift
exec "$@"
SH
chmod +x "$BAC/tmo/timeout"
assert_egal "fichier ordinaire" "$(if [ -L "$BAC/tmo/timeout" ]; then echo lien; elif [ -f "$BAC/tmo/timeout" ]; then echo "fichier ordinaire"; else echo absent; fi)" "le faux timeout est un fichier ordinaire, jamais un lien"
assert_egal "ELF" "$(head -c 4 "$REEL_TIMEOUT" | tail -c 3)" "le vrai timeout du conteneur est intact — binaire ELF, non remplacé"
CHEMIN_SAIN="$CHEMIN"; CHEMIN="$BAC/tmo:$CHEMIN"
...
assert_absent "$sortie" "[SUCCESS]" "aucun [SUCCESS] ne masque l'échec partiel"
CHEMIN="$CHEMIN_SAIN"; LISTE="web,data,monitoring"
...
titre "Traces héritées — coupées avant que le jeton ne soit lu"
sain; LISTE="web"; EXTRA=(CLUSTER_NS="web")
INTERPRETE=(-xv); lancer --yes; INTERPRETE=()
assert_code 0 "$CODE" "« bash -xv » : la trace est coupée, le script rend 0"
assert_absent "$sortie" "$JETON" "le jeton n'est pas tracé par -x"
assert_absent "$sortie" "$IDENT" "l'identifiant non plus"
sain; EXTRA=(CLUSTER_NS="web" SHELLOPTS=xtrace:verbose:allexport); lancer --yes; EXTRA=()
assert_code 0 "$CODE" "SHELLOPTS=xtrace:verbose:allexport hérité : le script rend 0"
assert_absent "$sortie" "$JETON" "le jeton n'est pas tracé non plus"
assert_absent "$sortie" "$IDENT" "l'identifiant non plus"
assert_non_vide "$(cat "$BAC/environ")" "l'environnement des processus fils a été relevé pendant l'appel"
assert_absent "$(cat "$BAC/environ")" "$JETON" "allexport coupé : le jeton n'est dans l'environnement d'aucun processus fils"
assert_absent "$(cat "$BAC/environ")" "$IDENT" "l'identifiant non plus"
EXTRA=(); LISTE="web,data,monitoring"
...
if [ -z "${SUITE_SOUS_TEST:-}" ]; then
    ...
    relancer "neutralisation des traces retirée" \
        "$(sed -e '/^set +xv +o allexport$/d' "$CIBLE" | mutant traces)"
    for i in "${!JUGES[@]}"; do
