...
# ce journal qui prouve l'ORDRE, et non la seule sortie du script : la règle SSH
# est posée, relue dans « ufw status », et l'activation vient après. Le faux ne
...
cat > "$BAC/ufw" <<'EOF'
#!/bin/sh
# Faux ufw : trace l'appel, tient l'état, ne touche à rien sur la machine.
...
# quoi les commandes « ufw default » ne seraient jamais passées et l'ordre des
# appels ne prouverait rien de leur sujet.
etat_neuf() {
...
# Garde : le faux doit être celui que PATH désigne. Un vrai ufw atteint par
# mégarde modifierait la machine qui exécute le test.
if [ "$(PATH="$BAC:$PATH" command -v ufw)" != "$BAC/ufw" ]; then
    saute_indisponible "faux ufw en tête de PATH" "PATH ne désigne pas $BAC/ufw"
...
lancer() {
    CODE=0
    SORTIE="$(PATH="$BAC:$PATH" "$@" 2>&1 </dev/null)" || CODE=$?
}
...
assert_avant() {
    ...
    if [ -n "$a" ] && [ -n "$b" ] && [ "$a" -lt "$b" ]; then
        ...
        ko "$3" "« $1 » (ligne ${a:-absente}) devrait précéder « $2 » (ligne ${b:-absente})"
    fi
}
...
assert_egal "" "$(cat "$UFW_LOG")" "aucune commande ufw avant la validation"
...
assert_egal "0" "$(grep -cv '^status' "$UFW_LOG" || true)" "--dry-run n'a passé aucune commande modificatrice"
...
assert_egal "" "$(cat "$UFW_LOG")" "aucune commande ufw n'est passée"
...
assert_egal "" "$(cat "$UFW_LOG")" "rien n'est appliqué avant la confirmation"

etat_neuf
lancer env ASSUME_YES=true bash "$CIBLE"
assert_code 1 "$CODE" "un ASSUME_YES hérité du parent ne confirme pas (décision 45)"
assert_egal "" "$(cat "$UFW_LOG")" "toujours aucune commande ufw"
...
#!/bin/sh
printf '%s\n' "\$*" >> "$APT_LOG"
[ "\$1" = install ] && cp "$BAC/ufw" "$BD/ufw"
exit 0
EOF
chmod +x "$BD/apt-get"
export APT_LOG="$BAC/apt.log"
etat_neuf; : > "$APT_LOG"; rm -f "$BD/ufw"
CODE=0
SORTIE="$(PATH="$BD:$BAC:/usr/bin:/bin" bash "$CIBLE" -y 2>&1 </dev/null)" || CODE=$?
...
assert_egal "" "$(cat "$UFW_LOG")" "aucune commande ufw avant le contrôle de distribution"
...
assert_egal "" "$(cat "$UFW_LOG")" "aucune commande ufw sans root"
