...
# passe et clavier interactif refusés, clé publique seule, port inchangé. Verrous
# contre le verrouillage : un compte non-root avec sudo et clé (décision 20), et
# « sshd -t » puis le rechargement, dont l'échec restaure l'état antérieur.
...
« sshd -t » valide la configuration avant tout rechargement ; si « sshd -t » ou
« systemctl reload ssh » échoue, l'état antérieur est restauré. Jamais de restart.
Refus si sshd_config n'inclut pas sshd_config.d/*.conf, ou si le compte nommé est
introuvable, root, hors de sudo, ou sans clé dans authorized_keys.

Codes : 0 appliqué, conforme ou --dry-run ; 1 root, distribution, inclusion,
compte, « sshd -t » ou rechargement en défaut ; 2 option ou valeur mal formée.
EOF
}
...
# Sans cet Include, le fichier déposé serait ignoré en silence. sshd ne tient pas
# compte de la casse des mots-clés : « include » vaut « Include ».
...
trap nettoyer EXIT
# Remet l'état antérieur : sauvegarde remise en place, ou dépôt retiré.
...
# Troisième verrou : sans rechargement, sshd n'a pas relu le fichier, et le
# laisser en place ferait dire « déjà conforme » à la relance.
