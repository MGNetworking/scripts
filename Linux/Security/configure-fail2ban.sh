...
# (décision 22) : un fichier déposé dans jail.d/, que les mises à jour du paquet
# ne remplacent pas — jail.conf et jail.local ne sont jamais touchés.
_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
while [ ! -f "$_dir/lib/common.sh" ] && [ "$_dir" != "/" ]; do _dir="$(dirname "$_dir")"; done
source "$_dir/lib/common.sh"

# Surchargeable pour que le test travaille dans un bac à sable.
JAIL_D="${FAIL2BAN_JAIL_D:-/etc/fail2ban/jail.d}"
PAQUET="fail2ban"; SERVICE="fail2ban"
FICHIER="$JAIL_D/mgnetworking-sshd.conf"
...
Dépose <jail.d>/mgnetworking-sshd.conf — [sshd] avec enabled = true — sans jamais
toucher jail.conf ni jail.local. Aucune valeur n'y est écrite : ce sont celles de
la distribution (décision 22). Fichier identique : rien n'est réécrit. Le service
n'est activé et redémarré que si quelque chose a changé, puis
...
dépôt, activation, redémarrage ou vérification en défaut, confirmation refusée ;
2 option ou valeur mal formée.
EOF
}
...
A_REDEMARRER="non"
if [ "$A_INSTALLER" = "oui" ] || [ "$A_DEPOSER" = "oui" ] || [ "$ACTIF" != "active" ]; then
    A_REDEMARRER="oui"
fi
A_FAIRE="non"
[ "$A_INSTALLER$A_DEPOSER$A_ACTIVER$A_REDEMARRER" = "nonnonnonnon" ] || A_FAIRE="oui"

{
    printf '\nÉtat actuel\n'
    if [ "$PAQUET_PRESENT" = "oui" ]; then printf '  paquet %s : installé\n' "$PAQUET"
    else printf '  paquet %s : absent, à installer par apt-get\n' "$PAQUET"; fi
    ...
    if [ "$A_FAIRE" = "non" ]; then printf '  aucune modification, puis vérification de la prison\n'
        ...
        printf '  fail2ban-client status sshd\n'
    fi
} >&2
