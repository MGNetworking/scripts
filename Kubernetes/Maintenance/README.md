...
| `cleanup-resources.sh` | **destructif** : supprime seulement les objets nommés un à un, `-n <namespace> <type>/<nom>` par objet ; ne cherche aucun candidat, jamais par label, motif, `--all` ni namespace entier. Liste blanche de types, comparée exactement, transmise à `kubectl` sous la forme canonique plurielle : `pods` (`pod`, `po`), `jobs`, `cronjobs` (`cj`), `deployments` (`deploy`), `replicasets` (`rs`), `statefulsets` (`sts`), `daemonsets` (`ds`), `services` (`svc`), `configmaps` (`cm`), `ingresses` (`ing`). Tout autre type rend 2 avant tout appel : virgule, `all`, majuscules, suffixe de groupe (`.v1`, `.apps`), types à portée cluster (namespaces, nœuds, PV, CRD, RBAC) pour lesquels `kubectl` ignorerait `-n`. **Exclus volontairement** : les Secrets, et les PVC, dont la suppression détruit les données quand la classe de stockage est en `reclaimPolicy: Delete` (cas de `local-path` sur K3s). Nom et namespace validés en forme DNS-1123, sinon 2 : aucun composant ne peut être lu comme une option. Namespaces kube-system, kube-public et kube-node-lease refusés en 1. Chaque cible relue par `kubectl get` avant la confirmation (`NotFound`, `Forbidden`, type inconnu, apiserver injoignable, délai dépassé nommés ; rien supprimé). Liste affichée, puis confirmation : `-y`/`--yes` seul confirme, un `ASSUME_YES` hérité est ignoré (décision 45), sans terminal ni `--yes` code 1. `delete --wait=false`, sans `--force` ni `--grace-period` ; un échec n'arrête pas les cibles suivantes ; relecture finale seule juge : bilan supprimées / en échec / non tentées, code 1, jamais `[SUCCESS]` (A78). Appels bornés par `--request-timeout` (30 s) et `timeout` (32 s). `--dry-run` : liste seule, aucun appel `kubectl`. Codes : 0 chaque cible relue absente, ou `--dry-run` ; 1 cible inexistante ou protégée, `kubectl` ou `timeout` absent, apiserver injoignable, droits insuffisants, délai dépassé, confirmation refusée, cible restante ; 2 option inconnue, cible mal formée, type hors liste, nom ou namespace invalide, aucune cible | utilisateur | **oui, sur le cluster** |
...
./Kubernetes/Maintenance/cleanup-resources.sh --dry-run -n web deploy/ancien -n web cm/ancien   # ce qui serait supprimé
./Kubernetes/Maintenance/cleanup-resources.sh -n web deploy/ancien -n web cm/ancien             # confirmation, suppression, relecture
```

## Risques

Aucun sur le cluster pour les six premiers scripts : lecture seule, `diagnostics.sh` compris, qui ne corrige rien. La
...
`cleanup-resources.sh` **supprime** : ce qu'on lui nomme disparaît, sans sauvegarde
(lancer d'abord `backup-resources.sh`). Supprimer un Deployment emporte ses ReplicaSets et
ses pods ; supprimer un pod géré le fait recréer par son contrôleur. Secrets et PVC ne
sont jamais acceptés : un PVC en `reclaimPolicy: Delete` emporte ses données. Seul le
format des arguments est vérifié hors cluster ; la liste blanche et les refus sont prouvés
avec un faux `kubectl`, jamais contre un vrai cluster (A96).
