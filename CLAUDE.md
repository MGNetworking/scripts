...
```text
Ansible/     roles | playbooks | inventory.example.yml | CADRAGE.md | README.md
...
date dans son cadrage, jamais supprimé sans décision de Maxime. La répartition
s'applique rôle par rôle : jusqu'au bilan du pilote `securite_base` et à la décision
de Maxime de poursuivre, les autres scripts Bash restent la référence et leurs tâches
se conduisent comme avant.
...
- Inventaire réel, `host_vars` réels et secrets hors Git : seuls les
  `*.example.yml` sont versionnés ; secrets par Ansible Vault ou fichiers locaux ignorés.
