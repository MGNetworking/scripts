...
while [ "${1:-}" != "" ]; do
    case "$1" in
        ...
        --supprimer-volumes) VOLUMES="true"; shift ;;
...
if [ -n "$INJOIGNABLE" ]; then
    ...
    info "[dry-run] Rien n'a été énuméré, rien ne sera supprimé. Les opérations seraient, dans cet ordre : conteneurs arrêtés ; réseaux inutilisés, un par un, hors réseaux protégés ; images sans étiquette${VOLUMES:+, volumes inutilisés}."
    exit 0
fi
relever
...
if [ "$VOLUMES" = "true" ]; then printf '    Inclus par --supprimer-volumes.\n'
...
VOLUMES_QUESTION=""
[ "$VOLUMES" != "true" ] || VOLUMES_QUESTION=", ainsi que $NV volume(s) et leurs données"
...
if [ "$VOLUMES" = "true" ] && [ "$NV" -gt 0 ]; then
    ...
    if ! confirm "Détruire ces $NV volume(s) et les données qu'ils portent ?"; then
        VOLUMES="false"; warn "Volumes conservés : leurs données ne sont pas touchées. Le reste du nettoyage se poursuit."
    fi
fi
...
[ "$VOLUMES" != "true" ] || while IFS= read -r nom; do [ -z "$nom" ] || supprimer volume rm "$nom"; done <<< "$LISTE_VOLUMES"
